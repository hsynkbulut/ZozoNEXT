...
class TopicBreakdownCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (topicBreakdown.isEmpty) return const SizedBox.shrink();

    final filteredTopics = selectedSubject == 'Tüm Dersler'
        ? topicBreakdown
        : Map.fromEntries(
            topicBreakdown.entries.where(
              (entry) => entry.key.startsWith('$selectedSubject:'),
            ),
          );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...filteredTopics.entries.map((entry) {
              final parts = entry.key.split(': ');
              final subject = parts[0];
              final topic = parts.length > 1 ? parts[1] : entry.key;
